...
qmap = [0, 1, 2, 3] # mapping on Tuna-9 qubits
...
def get_golden(testcases):
    ...
    for init_testcase in testcases:
        circ = QuantumCircuit(len(qmap), len(qmap))
...
def make_measures(backend: Runner, measures, testcases, num_shots=2048):
    freqs_list = []
    for init_testcase in testcases:
        freqs = measure(backend, measures, init_testcase, len(qmap), num_shots)
...
ops = ops_#transpile_measure_operators(ops_, 9, qmap)
backend = Runner("QX emulator")
print("Make measures...")
...
for k, freqs in enumerate(freqs_list):
    print(f"Testcase {k}: {testcases_num_qubits[k]} qubits")
